...
class Syncthing:
    ...
    def __init__(self, gui_port: int | None = None, apikey: str | None = None):
    ...
    def start(self, log_fn=lambda m: None) -> None:
        ...
        args = [str(exe), "serve", "--home", str(_HOME_DIR), "--no-browser"]
    ...
    def _get(self, path: str, timeout: float = 5.0):
        req = urllib.request.Request(f"http://{self.gui_address}{path}",
