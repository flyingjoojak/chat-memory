...
// 이슈 URL 생성. copied=클립보드 복사 성공 여부(본문 안내 문구가 달라짐).
function issueUrl(report: SchemaReport, reportJson: string, copied: boolean): string {
  ...
  const note = copied
    ...
    : "설정 화면의 신고서 내용을 복사해 여기에 붙여넣어 주세요(대화 내용 없음).\n\n```json\n(붙여넣기)\n```\n"
...
export function SchemaReportSection() {
  ...
  const openIssue = async () => {
    ...
    setCopyFailed(!ok)
    if (ok) flashCopied()
    window.open(issueUrl(report, reportJson, ok), "_blank", "noopener,noreferrer")
  }
