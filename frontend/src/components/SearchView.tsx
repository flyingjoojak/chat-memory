    ...
    <div className="relative inline-flex rounded-full border bg-card p-0.5 text-xs select-none">
      ...
      {options.map((o, i) => (
        <button key={o} onClick={() => onChange(i as 0 | 1)}
          className={`relative z-10 min-w-[80px] rounded-full px-3 py-1.5 transition-colors ${value === i ? "text-primary font-semibold" : "text-muted-foreground"}`}>
          {o}
        </button>
      ))}
    </div>
          ...
          <div className="mt-2 flex flex-wrap items-center gap-2 text-xs text-muted-foreground">
            <label className="inline-flex items-center gap-1">이후
              <input type="date" value={since} onChange={(e) => { setSince(e.target.value); run() }}
                className="rounded-md border bg-card px-1.5 py-1 tabular-nums outline-none shadow-sm [color-scheme:light_dark]" /></label>
