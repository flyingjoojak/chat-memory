        ...
        <div className="mt-6 space-y-3">
          {err && (
            <div className="flex flex-col items-center gap-3 rounded-xl border border-destructive/30 bg-destructive/5 p-6 text-center text-sm">
              <span className="inline-flex items-center gap-2 text-destructive"><AlertTriangle className="size-4" />{t("onboarding.errorTitle")}</span>
              <span className="text-muted-foreground">{t("onboarding.errorDetail")}</span>
              <button onClick={load} className="rounded-lg bg-primary px-4 py-1.5 text-sm font-medium text-primary-foreground">{t("common.retry")}</button>
            </div>
          )}
          {!models && !err && (
            ...
            <button key={m.model} type="button" disabled={!!picking} onClick={() => pick(m)}
              ...
              <div className="min-w-0 flex-1">
                ...
                <div className="mt-1.5 text-[11.5px] text-muted-foreground tabular-nums">
                  {m.note} · {t("onboarding.modelMeta", { size: m.size_gb, ram: m.ram_gb })}
                </div>
              </div>
              {picking === m.model
                ? <Loader2 className="size-5 shrink-0 animate-spin text-primary" />
                : <span className="shrink-0 rounded-lg bg-primary px-3 py-1.5 text-sm font-medium text-primary-foreground transition-opacity group-hover:opacity-90">{t("onboarding.selectButton")}</span>}
            </button>
          ))}
        </div>
