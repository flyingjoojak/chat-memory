...
export function SourceFilter({ available, selected, onChange }: Props) {
  ...
  const summary = selected.size >= total
    ? t("search.sourceAll")
    : selected.size === 1 ? label([...selected][0]) : t("search.sourceCount", { n: selected.size })
  ...
  return (
    <div className="relative" onKeyDown={(e) => { if (e.key === "Escape" && open) { e.stopPropagation(); close() } }}>
      <button ref={triggerRef} type="button" onClick={() => setOpen((o) => !o)}
        ...
        {t("search.sourceLabel")} <span className="font-medium text-foreground">{summary}</span>
      ...
      {open && (
        <>
          ...
          <div id={panelId} role="group" aria-label={t("search.sourceFilterAria")}
