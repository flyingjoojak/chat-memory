...
from engram import mcp_register as R
...
def test_frozen_registers_self_with_mcp_flag(monkeypatch):
    ...
    monkeypatch.setattr(R.sys, "executable", r"C:\app\engram-backend.exe", raising=False)
    cmd, args = R.mcp_command()
    assert cmd == r"C:\app\engram-backend.exe"
    assert args == ["--mcp"]


def test_dev_prefers_console_script(monkeypatch):
    monkeypatch.setattr(R.sys, "frozen", False, raising=False)
    monkeypatch.setattr(R.shutil, "which", lambda name: "/usr/local/bin/engram-mcp")
    cmd, args = R.mcp_command()
    assert cmd == "/usr/local/bin/engram-mcp"
    assert args == []
...
def test_dev_falls_back_to_python_module(monkeypatch):
    ...
    assert args == ["-m", "engram.mcp_server"]
